...
export default function TrustBadges({
  ...
  return (
    <section className="border-b border-sand bg-white">
      ...
      {/* Mobile: swipe carousel + dots — isang badge bawat slide, NAKA-CENTER
          ang icon at mensahe. JS snap (useSwipeFallback) ang nagpapadapo sa
          bawat slide; walang CSS scroll-snap (may iOS freeze bug iyon). */}
      <div className="md:hidden py-6">
        ...
        >
          {badges.map((b) => (
            <div
              key={b.title}
              className="w-full shrink-0 snap-center flex items-center justify-center gap-4 px-8 text-left"
            >
              <span className="text-ink shrink-0">{ICONS[b.icon] ?? ICONS.sofa}</span>
              <div className="text-sm text-ink leading-snug">
                <p>{b.title}</p>
